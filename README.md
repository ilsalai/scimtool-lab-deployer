...
- **Pre-flight checks** — Azure CLI, PowerShell version, internet reachability, subscription state
- **Guided Azure login** and active-subscription validation
- **Runs the upstream bootstrap** to provision the Container App, VNet, and secrets
- **Auto-retry on the subnet delegation error** — detects `SubnetDelegationError`, delegates `aca-infra` to `Microsoft.App/environments`, and re-runs the bootstrap
...
When the upstream bootstrap starts asking questions, **just press Enter on everything** — the defaults are what you want. Do not type custom resource group or app names; the NSG fix in step 6 relies on the default naming convention.
...
You'll see something like `SubnetDelegationError: Subnet '<name>/aca-infra' is not delegated to 'Microsoft.App/environments'`. This is a known race in the upstream bootstrap — the subnet gets created but the delegation doesn't stick on the first pass.

**The script handles this automatically:** it detects the error in the bootstrap transcript, runs

```powershell
az network vnet subnet update --resource-group <rg> --vnet-name <app>-vnet --name aca-infra --delegations Microsoft.App/environments
```

and re-runs the bootstrap. On the retry, the existing resources are reused and deployment completes. If the error happens a second time, the script bails out and suggests deleting the resource group and starting over — in practice this is always enough.
...
The upstream `bootstrap.ps1` occasionally gains new prompts. The safe answer is always **Enter** (accept default) unless you know what you're changing. Custom names will break the NSG-fix step because it derives the NSG name from the app name.
